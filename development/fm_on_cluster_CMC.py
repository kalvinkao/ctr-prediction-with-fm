...
conf = sc._conf.setAll([('spark.executor.memory', '6g'), ('spark.driver.memory','6g'), ('spark.executor.cores', '4'), ('spark.rdd.compress', 'True')])
sc.stop()
...
dataFolder = "gs://w261bucket/"
#dataFolder = "data/"
resultsFolder = "gs://w261bucket/results/"
...
excludeRDD, TestRDD, TrainRDD = fullRDD.randomSplit([1-2*splits, splits, splits], seed = randSeed)
#TestRDD, TrainRDD = fullRDD.randomSplit([0.25, 0.75], seed = 1)
#TrainRDD.cache()
#TestRDD.cache()
...
def vectorizeCV(fullDF, sampleDF, minDocFrec):
    ...
    cv = CountVectorizer(minDF=minDocFrec, inputCol="raw", outputCol="features", binary=True)
...
# call functions
parsedRDD = TrainRDD.map(parseCV).cache()
if sampleFeats < 1.0:
    sampleRDD = parsedRDD.sample(False, sampleFeats, randSeed)
    vectorizedDF, cvModel = vectorizeCV(parsedRDD.toDF(), sampleRDD.toDF(), minDocFrec)
else:
    vectorizedDF, cvModel = vectorizeCV(parsedRDD.toDF(), None, minDocFrec)
...
start = time.time()
losses, b_br, w_br, V_br = iterateSGD(vectorizedRDD, k, b, w, V, nIter, learningRate = 0.1, useReg = useReg)
...
smallUnlabeledRDD = unlabeledRDD.sample(False, scoreTest, randSeed)
