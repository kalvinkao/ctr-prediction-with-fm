...
def fm_get_p(x, w):
    ...
    if type(x) == pyspark.mllib.linalg.SparseVector :
        W = w[x.indices] # tried  w[x.index]  #old 
        X = x.values
...
def trainFM_parallel_sgd (sc, data, iterations=50, iter_sgd= 5, alpha=0.01, regParam=0.01, factorLength=4,\
   ...
   # print('train_x', train_X.take(1))
    ...
    #print("jen's test", train_XY.take(2))
    #return("jen's test2", wsub)

    for i in range(iterations):
        wb = sc.broadcast(w)
        wsub = train_XY.map(lambda X_y: sgd_subset(X_y[0], X_y[1], wb.value, iter_sgd, alpha, regParam))
        #print("jen's test wsub", wsub.collect())
        w = wsub.mean()
